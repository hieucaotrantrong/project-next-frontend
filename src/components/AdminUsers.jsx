...
const AdminUsers = () => {
    ...
    const fetchUsers = async () => {
        try {
            ...
            const res = await axios.get('https://project-next-backend.onrender.com/api/admin/users', {
    ...
    const saveEdit = async (id) => {
        try {
            const token = localStorage.getItem('token');
            await axios.put(`https://project-next-backend.onrender.com/api/admin/users/${id}`, form, {
                headers: token ? { Authorization: `Bearer ${token}` } : {}
            });
            await fetchUsers();
            cancelEdit();
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`https://project-next-backend.onrender.com/api/admin/users/${id}`, {
                headers: token ? { Authorization: `Bearer ${token}` } : {}
            });
            await fetchUsers();
    ...
    const createUser = async (e) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.post('https://project-next-backend.onrender.com/api/admin/users', newUser, {
                headers: token ? { Authorization: `Bearer ${token}` } : {}
            });
